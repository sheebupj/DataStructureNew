...
class AVLTree {
    ...
    class Node {
        ...
        Node(int d) {
            key = d;
            height = 1; // New nodes are added at leaf level (height 1)
        }
    }
    ...
    Node insert(Node node, int key) {
        ...
        // Case 1: Left Left (Single Right Rotate)
        if (balance > 1 && key < node.left.key)
            return rightRotate(node);

        // Case 2: Right Right (Single Left Rotate)
        if (balance < -1 && key > node.right.key)
            return leftRotate(node);

        // Case 3: Left Right (Double Rotate: Left then Right)
        if (balance > 1 && key > node.left.key) {
            node.left = leftRotate(node.left);
            return rightRotate(node);
        }

        // Case 4: Right Left (Double Rotate: Right then Left)
        if (balance < -1 && key < node.right.key) {
